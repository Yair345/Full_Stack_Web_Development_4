      ...
      if (
        ...
        if (endIndex >= currentIndex + blockLength) {
          ...
          while (
            ...
            updatedBlocks[j] = {
              ...updatedBlocks[j],
              style: { ...updatedBlocks[j].style, backgroundColor: "yellow" },
            };
      ...
      if (blockText.includes(searchText)) {
        ...
        for (let i = 0; i < parts.length; i++) {
          ...
          if (i < parts.length - 1) {
            ...
            newBlocks.push({ text: replaceText, style: blockStyle });
          }
        }
      } else {
    ...
    for (let i = 0; i < textBlocks.length; i++) {
      ...
      if (
        ...
        if (endIndex < currentIndex + blockLength) {
          ...
          let j = i + 1;
          let charactersToSkip = currentIndex + blockLength - startIndex;

          while (j < textBlocks.length) {
            const nextBlock = textBlocks[j];
            const nextBlockLength = nextBlock.text.length;

            if (charactersToSkip + nextBlockLength > searchText.length) {
              ...
              // Add all subsequent blocks
              newBlocks = [...newBlocks, ...textBlocks.slice(j + 1)];
              break;
            }
